...
class SocialTestMixin(object):
    ...
    def persona_login(self, mock_requests, verifier_data=None, next_url=None):
        """
        Mock a login to Persona and return the response.

        Keyword Arguments:
        mock_requests - a requests_mock mock
    ...
    def github_login(
        ...
        mock_requests.get(
            ...
            json=self.github_email_data if email_data is None else email_data)

        data = {'process': process}
        state = 'github_state'
        self.set_github_oauth2_state(data, state)

        url = reverse('github_callback', locale=settings.WIKI_DEFAULT_LANGUAGE)
    ...
    def set_github_oauth2_state(self, data, state):
        session = self.client.session
        session['socialaccount_state'] = (data, state)
        session.save()
